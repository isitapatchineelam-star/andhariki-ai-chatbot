...
def chat():
    try:
        ...
        if not key:
            return jsonify({"reply":"Render lo API KEY ledu! Environment lo pettu!"})
        ...
        return jsonify({"reply": f"Real Error: {str(e)[:500]}"})
